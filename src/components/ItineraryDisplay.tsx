...
import { ItineraryResponse, Activity } from "@/services/geminiService";
import { generateGoogleMapsUrl } from "@/lib/utils";

interface ItineraryDisplayProps {
  itinerary: ItineraryResponse;
  onActivityClick?: (dayIndex: number, activity: Activity) => void;
}

const ItineraryDisplay = ({ itinerary, onActivityClick }: ItineraryDisplayProps) => {
            ...
            <div className="divide-y divide-gray-100">
              {day.activities.map((activity, index) => (
                <div
                  ...
                  onClick={() => onActivityClick?.(dayIndex, activity)}
                  className="p-8 hover:bg-gradient-to-r hover:from-orange-50/50 hover:to-red-50/30 transition-all duration-300 group relative overflow-hidden cursor-pointer"
                >
                    ...
                    <div className="flex-grow space-y-4">
                      <div className="flex justify-between items-start">
                        <div className="space-y-3 flex-grow">
                              ...
                              {typeof activity.rating === 'number' && activity.rating > 0 && (
                                <div className="flex items-center space-x-1 mt-1">
                                  {[...Array(5)].map((_, i) => (
                                    <Star
                                      key={i}
                                      className={`h-4 w-4 ${i < activity.rating! ? "text-yellow-400 fill-yellow-400" : "text-gray-200"}`}
                                    />
                                  ))}
                                </div>
                              )}
                          ...
                          {activity.review && (
                            <div className="pl-7 pt-2 italic text-gray-500 text-base flex items-start gap-2">
